/*
       (|x2-x1  y2-y1   z2-z1|)
V = abs(|x3-x1  y3-y1   z3-z1|)
       (|x4-x1  y4-y1   y4-y1|)
    ---------------------------
                6
*/

// Función para calcular el volumen local de un tetraedro dado por sus cuatro vértices
...
{
    float A = abs((x2 - x1) * (y3 - y1) * (z4 - z1) + (y2 - y1) * (z3 - z1) * (x4 - x1) + (z2 - z1) * (x3 - x1) * (y4 - y1) - (z2 - z1) * (y3 - y1) * (x4 - x1) - (x2 - x1) * (z3 - z1) * (y4 - y1) - (y2 - y1) * (x3 - x1) * (z4 - z1)) / 6;
...
// Función para calcular el jacobiano local de un tetraedro dado por sus cuatro vértices
...
{
    float J = (x2 - x1) * (y3 - y1) * (z4 - z1) + (x3 - x1) * (y4 - y1) * (z2 - z1) + (x4 - x1) * (y2 - y1) * (z3 - z1) - (x4 - x1) * (y3 - y1) * (z2 - z1) - (x3 - x1) * (y2 - y1) * (z4 - z1) - (x2 - x1) * (y4 - y1) * (z3 - z1);
...
// Función para calcular la matriz A local para un elemento
void calculate_local_A(Matrix *A, float x1, float y1, float z1, float x2, float y2, float z2, float x3, float y3, float z3, float x4, float y4, float z4)
{
    ...
    A->set((x4 - x1) * (z3 - z1) - (x3 - x1) * (z4 - z1), 0, 1);
    A->set((x3 - x1) * (y4 - y1) - (x4 - x1) * (y3 - y1), 0, 2);
    A->set((y4 - y1) * (z2 - z1) - (y2 - y1) * (z4 - z1), 1, 0);
    A->set((x2 - x1) * (z4 - z1) - (x4 - x1) * (z2 - z1), 1, 1);
    A->set((x4 - x1) * (y2 - y1) - (x2 - x1) * (y4 - y1), 1, 2);
    A->set((y2 - y1) * (z3 - z1) - (y3 - y1) * (z2 - z1), 2, 0);
    A->set((x3 - x1) * (z2 - z1) - (x2 - x1) * (z3 - z1), 2, 1);
    A->set((x2 - x1) * (y3 - y1) - (x3 - x1) * (y2 - y1), 2, 2);
}

// Función para crear la matriz K local para un elemento específico
void create_local_K(Matrix *K, short element_id, Mesh *M)
{
    ...
    cout << "\t\tVolume for Element " << element_id + 1 << ": " << Volume << "\n";
    cout << "\t\tJacobian for Element " << element_id + 1 << ": " << J << "\n";
    ...
    cout << "\t\tLocal matrix created for Element " << element_id + 1 << ": ";
    K->show();
    cout << "\n";
}

// Función para crear el vector b local para un elemento específico
void create_local_b(Vector *b, short element_id, Mesh *M)
{
    ...
    cout << "\t\tLocal vector created for Element " << element_id + 1 << ": ";
    b->show();
    cout << "\n";
}

// Función para crear los sistemas locales (matrices K y vectores b) para todos los elementos
...
// Función para ensamblar la matriz global K a partir de las matrices locales K
...
// Función para ensamblar el vector global b a partir de los vectores locales b
...
// Función para ensamblar la matriz K y el vector b globales
...
// Función para aplicar las condiciones de contorno de Neumann al vector b
...
// Función para añadir una columna al lado derecho de la ecuación
...
// Función para aplicar las condiciones de contorno de Dirichlet
...
// Función para fusionar los resultados con las condiciones de contorno de Dirichlet
